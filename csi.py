...
def is_orphan(centroid, mapping):
    return centroid not in mapping.values()


def one_way_centroid_index(a, b):
    mapping = calculate_mapping(a, b)
    return sum(is_orphan(centroid, mapping) for centroid in b)


def centroid_index(a, b):
    if a is None or b is None:
        return None
    return max(
        one_way_centroid_index(a, b),
        one_way_centroid_index(b, a),
    )
